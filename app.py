...
if st.session_state.user_role == "administrator":
    st.title(f"⚙️ Panel Administrator - {st.session_state.user_full_name}")
...
if not os.path.exists(TEMPLATE_PATH):
    st.error(f"⚠️ BERKAS UTAMA HILANG: File `{TEMPLATE_PATH}` wajib diletakkan di root folder aplikasi!")
    st.stop()
...
tab1, tab2, tab3 = st.tabs(["📋 FASE 1: Struktur Kerangka", "📊 FASE 2: Angka Hasil Ukur", "🗄️ PUSAT ARSIP DIGITAL"])
...
with tab2:
    ...
    if st.session_state.get("fase1_extracted"):
        ...
        st.warning("⚠️ Perhatian: Ekstrak dan kunci data topologi rute di Tab Fase 1 terlebih dahulu agar form input angka ini dapat terbuka.")

with tab3:
    st.header("🗄️ Brankas Digital Manajemen database Proyek")
